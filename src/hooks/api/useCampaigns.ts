...
import type { Campaign, CreateCampaignRequest, UpdateCampaignRequest, CampaignsQueryParams } from '@/lib/api/types/campaigns.types';

export const campaignsKeys = {
  all: ['campaigns'] as const,
  lists: () => [...campaignsKeys.all, 'list'] as const,
  list: (params: CampaignsQueryParams) => [...campaignsKeys.lists(), params] as const,
  details: () => [...campaignsKeys.all, 'detail'] as const,
  detail: (id: string) => [...campaignsKeys.details(), id] as const,
};

const campaignsApi = {
  getAll: async (params: CampaignsQueryParams): Promise<PaginatedResponse<Campaign>> => {
    const queryString = buildQueryString(params);
    return await apiClient.get(`campaigns${queryString}`).json<PaginatedResponse<Campaign>>();
  },
  ...
  update: async (id: string, data: UpdateCampaignRequest): Promise<Campaign> => {
    return await apiClient.patch(`campaigns/${id}`, { json: data }).json<Campaign>();
  },
...
export const useCampaigns = (params: CampaignsQueryParams = {}): UseQueryResult<PaginatedResponse<Campaign>, Error> => {
  return useQuery({
...
export const useCreateCampaign = (): UseMutationResult<Campaign, Error, CreateCampaignRequest> => {
...
export const useUpdateCampaign = (): UseMutationResult<Campaign, Error, { id: string; data: UpdateCampaignRequest }> => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }) => campaignsApi.update(id, data),
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: campaignsKeys.lists() });
      queryClient.setQueryData(campaignsKeys.detail(variables.id), data);
    },
  });
};
