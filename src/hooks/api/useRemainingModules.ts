...
const gsmDevicesApi = {
  getAll: async (params: GsmDevicesQueryParams): Promise<PaginatedResponse<GsmDevice>> => { const queryString = buildQueryString(params); return await apiClient.get(`gsm-devices${queryString}`).json<PaginatedResponse<GsmDevice>>(); },
...
export const useGsmDevices = (params: GsmDevicesQueryParams = {}) => useQuery({ queryKey: gsmDevicesKeys.list(params), queryFn: () => gsmDevicesApi.getAll(params), staleTime: 30 * 1000 });
export const useGsmDevice = (id: string) => useQuery({ queryKey: gsmDevicesKeys.detail(id), queryFn: () => gsmDevicesApi.getById(id), enabled: !!id, staleTime: 30 * 1000 });
export const useCreateGsmDevice = () => { const qc = useQueryClient(); return useMutation({ mutationFn: gsmDevicesApi.create, onSuccess: () => qc.invalidateQueries({ queryKey: gsmDevicesKeys.lists() }) }); };
export const useUpdateGsmDevice = () => { const qc = useQueryClient(); return useMutation({ mutationFn: ({ id, data }: { id: string; data: UpdateGsmDeviceRequest }) => gsmDevicesApi.update(id, data), onSuccess: (data, variables) => { qc.invalidateQueries({ queryKey: gsmDevicesKeys.lists() }); qc.setQueryData(gsmDevicesKeys.detail(variables.id), data); } }); };
export const useDeleteGsmDevice = () => { const qc = useQueryClient(); return useMutation({ mutationFn: gsmDevicesApi.delete, onSuccess: (_, id) => { qc.invalidateQueries({ queryKey: gsmDevicesKeys.lists() }); qc.removeQueries({ queryKey: gsmDevicesKeys.detail(id) }); } }); };
