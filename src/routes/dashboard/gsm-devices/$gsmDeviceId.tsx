...
import {
  ...
  useDeleteGsmDevice,
  useGoipStatus,
  useDeleteGsmDevicePort,
} from '@/hooks/api/useGsmDevices'
...
function GsmDeviceDetailPage() {
  ...
  const { data: portsData, isLoading: isPortsLoading } =
    useGsmDevicePorts(gsmDeviceId)
  const { data: goipStatus } = useGoipStatus(gsmDeviceId)
  const { mutate: deleteGsmDevice, isPending: isDeletePending } =
    useDeleteGsmDevice()
  const { mutate: deleteGsmDevicePort, isPending: isDeletePortPending } =
    useDeleteGsmDevicePort()
  ...
  const mergedPortsData = portsData?.map((port) => {
    const goipLine = goipStatus?.lines.find(
      (line) => line.id === port.portNumber,
    )
    return {
      ...port,
      goipStatus: goipLine,
  ...
  const handleDelete = () => {
    deleteGsmDevice(gsmDeviceId, {
      onSuccess: () => {
        toast.success(
          t('gsmDevices.deleteSuccess', 'GSM Device deleted successfully'),
        )
        navigate({
          to: '/dashboard/gsm-devices',
          search: { page: 1, limit: 10, view: 'table' },
        })
      },
      onError: (error: any) => {
        toast.error(
          error?.message ||
            t('gsmDevices.deleteError', 'Failed to delete GSM device'),
        )
      },
    })
  }
                    ...
                    <TableHeader className="bg-gradient-to-r from-primary/5 to-primary/10">
                      <TableRow className="hover:bg-transparent">
                        <TableHead className="font-semibold text-primary">
                          {t('gsmDevices.ports.number', 'Port #')}
                        </TableHead>
                        <TableHead className="font-semibold text-primary">
                          {t('gsmDevices.ports.phoneNumber', 'Phone Number')}
                        </TableHead>
                        <TableHead className="font-semibold text-primary">
                          {t('gsmDevices.ports.status', 'Status')}
                        </TableHead>
                        <TableHead className="font-semibold text-primary">
                          {t('gsmDevices.ports.gsmSim', 'GSM SIM')}
                        </TableHead>
                        <TableHead className="font-semibold text-primary">
                          {t('gsmDevices.ports.gsmStatus', 'GSM Status')}
                        </TableHead>
                        <TableHead className="font-semibold text-primary">
                          {t('gsmDevices.ports.voip', 'VOIP')}
                        </TableHead>
                        <TableHead className="font-semibold text-primary">
                          {t('gsmDevices.ports.gsmSignal', 'Signal')}
                        </TableHead>
                        <TableHead className="font-semibold text-primary">
                          {t('gsmDevices.ports.gsmOperator', 'Operator')}
                        </TableHead>
                        <TableHead className="font-semibold text-primary">
                          {t('gsmDevices.ports.updatedAt', 'Last Updated')}
                        </TableHead>
                        <TableHead className="font-semibold text-primary">
                        ...
                        mergedPortsData.map((port) => (
                          <TableRow key={port.id}>
                            ...
                            <TableCell>
                              <Badge
                                variant={
                                  port.status === GsmPortStatus.AVAILABLE
                                    ? 'default'
                                    : port.status === GsmPortStatus.ERROR
                                      ? 'destructive'
                                      : 'secondary'
                                }
                              >
                            ...
                            <TableCell>
                              <Badge
                                variant={
                                  port.goipStatus?.status_line === 'Y'
                                    ? 'default'
                                    : 'secondary'
                                }
                              >
                                {port.goipStatus?.status_line || '-'}
                              </Badge>
                            </TableCell>
                            <TableCell>
                              {port.goipStatus?.gsm_signal || '-'}
                            </TableCell>
                            <TableCell>
                              {port.goipStatus?.gsm_cur_oper || '-'}
                            </TableCell>
                            <TableCell>
                              {port.updatedAt
                                ? format(new Date(port.updatedAt), 'PP p')
                                : '-'}
                            </TableCell>
                            <TableCell>
                              <DropdownMenu>
        ...
        <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {t(
                  'gsmDevices.deleteConfirmDescription',
        ...
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {t(
                  'gsmDevices.ports.deleteConfirmDescription',
                    ...
                    deleteGsmDevicePort(
                      { id: deletingPort.id },
                      {
                        onSuccess: () => {
                          toast.success(
                            t(
                              'gsmDevices.ports.deleteSuccess',
